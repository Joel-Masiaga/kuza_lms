...
class LessonDetailView(View):
    def get(self, request, pk):
        ...
        # Get all lessons read by the user
        read_lesson_ids = lesson.module.course.modules.all().prefetch_related('lessons').filter(lessons__read_by_users=user).values_list('lessons__id', flat=True)
        ...
        return render(request, 'home/lesson.html', {
            ...
            'read': read,  # Indicates whether the lesson is marked as read by the user
            'quiz_attempt': quiz_attempt,  # Pass quiz attempt data to the template
            'read_lesson_ids': read_lesson_ids  # Pass the list of read lesson IDs to the template
        })
    ...
    def post(self, request, pk):
        ...
        # Redirect to next lesson or stay on the current one
